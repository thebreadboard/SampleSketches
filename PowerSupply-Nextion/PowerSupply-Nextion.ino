...
int16_t encoderLastVolts, encoderValueVolts, encoderLastAmps, encoderValueAmps;
...
void initEncoders()
{
  encoderVolts = new ClickEncoder(2, 3, 4);
  encoderAmps = new ClickEncoder(5, 6, 7);
...
{
  encoderValueVolts += encoderVolts->getValue();
  encoderValueAmps += encoderAmps->getValue();
  ClickEncoder::Button encBtnVolts = encoderVolts->getButton();
  ClickEncoder::Button encBtnAmps = encoderAmps->getButton();

  if (encoderValueVolts != encoderLastVolts)
  {
    encoderLastVolts = encoderValueVolts;
    Serial.print("encoderVolts Value: ");
    Serial.println(encoderValueVolts);
  }
  if (encoderValueAmps != encoderLastAmps)
  {
    encoderLastAmps = encoderValueAmps;
    Serial.print("encoderAmps Value: ");
    Serial.println(encoderValueAmps);
  }

// Check Buttons
  if (encBtnVolts != ClickEncoder::Open)
  {
    Serial.print("encBtnVolts: ");
    switch (encBtnVolts) {
      ...
      case ClickEncoder::DoubleClicked:
        ...
        Serial.println((encoderVolts->getAccelerationEnabled()) ? "enabled" : "disabled");
  ...
  if (encBtnAmps != ClickEncoder::Open)
  {
    ...
    switch (encBtnAmps) {
      ...
      case ClickEncoder::DoubleClicked:
        ...
        Serial.print("  Acceleration is ");
        Serial.println((encoderAmps->getAccelerationEnabled()) ? "enabled" : "disabled");
        break;
    }
  }
