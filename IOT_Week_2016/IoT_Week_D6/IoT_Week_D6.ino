...
bool autoFan = true; // fan is in auto mode
bool fanManState = false; // state of fan when in manual control
...
const char* topicSubscribeAuto = "thebreadboard/autoFan";
const char* topicSubscribeManState = "thebreadboard/on";
...
void loop()
{
	...
	client.yield(50); // test for recieved messages
}
...
void messageArrivedHigh(MQTT::MessageData& md)
{
	MQTT::Message &message = md.message;
        ...
        char p[message.payloadlen + 1];
        memcpy(p, (char*)message.payload, message.payloadlen);
        p[message.payloadlen] = NULL;	
         sprintf(printbuf, "%s", p);

	fanTempOn = atof(printbuf);
	Serial.print("setPointHigh now: "); Serial.print(printbuf);
	Serial.print(" Payload Lengh: "); Serial.println(message.payloadlen);
}
// This function is called via a seperate listener, not the main loop
void messageArrivedLow(MQTT::MessageData& md)
{
	...
	Serial.print("setPointLow now: "); Serial.print(printbuf);
	Serial.print(" Payload Lengh: "); Serial.println(message.payloadlen);
}
// This function is called via a seperate listener, not the main loop
void messageArrivedAuto(MQTT::MessageData& md)
{
	MQTT::Message &message = md.message;
	...
	autoFan = atoi(printbuf);
	Serial.print("Auto now: "); Serial.print(printbuf);
	Serial.print(" Payload Lengh: "); Serial.println(message.payloadlen);
}
// This function is called via a seperate listener, not the main loop
void messageArrivedManFan(MQTT::MessageData& md)
{
	MQTT::Message &message = md.message;
	...
	fanManState = atoi(printbuf);
	Serial.print("Man state now: "); Serial.print(printbuf);
	Serial.print("  Payload Lengh: "); Serial.println(message.payloadlen);
}
...
void connectMQTT()
{
	...
	rc = client.subscribe(topicSubscribeAuto, MQTT::QOS0, messageArrivedAuto);
	if (rc != 0)
	{
		FailedSubscription = true;
	}
	rc = client.subscribe(topicSubscribeManState, MQTT::QOS0, messageArrivedManFan);
	if (rc != 0)
	{
		FailedSubscription = true;
	}
...
void updateScreen()
{
	if (ScreenTime < millis())
	{
		...
		ScreenTime = millis() + ScreenDelay;
	        ...
	        if (autoFan == true) 
                { 
                     sharp96.text(40, 78, "Auto-");
                     (fanState == false)? sharp96.text(70, 87, "OFF"): sharp96.text(70, 87, "ON "); 
                }
                else
                {
                       sharp96.text(40, 78, "Man--");
                      (fanManState == false)? sharp96.text(70, 87, "OFF"): sharp96.text(70, 87, "ON ") ;
                }
