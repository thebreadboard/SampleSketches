...
#define DAC8571_ADD 0x4C // allowing for RW bit being added
#define DAC8571_CTR 0x34 // broadcast to all dac channels
...
#define TWI_FREQ_DAC 1000000L // 1Mhz
...
void setup()
{
  Wire.begin(DAC8571_ADD);
  // Arduino only....
  // TWBR = ((CPU_FREQ / TWI_FREQ_DAC) - 16) / 2;

}

void loop()
{
 byte x=0; // rely on byte roll over after 256
while(1)
{
   SetDAC(DAC8571_ADD, DACCh0, Sin_tab[x++]);
  }
}

//Works with DAC8571 and DAC8574
void SetDAC(byte DAC857xAddr, byte DACCh, unsigned int value)
{
  Wire.beginTransmission(DAC857xAddr); // transmit to device 
  Wire.write(DACCh); 
  Wire.write(value >> 8);  // Send MSB
  Wire.write(value);        //  Send LSB
